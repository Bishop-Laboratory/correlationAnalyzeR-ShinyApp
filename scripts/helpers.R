...
MDFtoTERM2GENE <- function(MDF, GSEA_Type, species = "hsapiens") {
  ...
  GSEA_Type <- gsub(GSEA_Type,
    pattern = "miRNAs",
    replacement = "miRNA targets", perl = TRUE
  )
  GSEA_Type <- gsub(GSEA_Type,
    pattern = "Transcription factors",
    replacement = "TF targets", perl = TRUE
  )
  GSEA_Type <- gsub(GSEA_Type,
    pattern = "Biological process",
    replacement = "GO:BP", perl = TRUE
  )
  GSEA_Type <- gsub(GSEA_Type,
    pattern = "Cellular component",
  ...
  if ("Basic" %in% GSEA_Type) {
    categories <- c(
      categories, "Hallmark", "Perturbations", "BioCarta",
...
getPhBreaks <- function(mat, palette = NULL) {
  ...
  n <- length(palette)
  breaks <- c(
    seq(min(mat), 0, length.out = ceiling(n / 2) + 1),
...
symbolConverter <- function(symbolVec, species, pool) {
  ...
  if (is.null(avgenes)) {
    stop(
      "Unable to connect to database. Please contact package maintainer",
  ...
  checkTheGene <- function(geneToCheck, aliasSymbol) {
    ...
    if (is.na(newGene)) {
      ...
      if (!length(aliases$alias_symbol)) {
        ...
        resDF <- data.frame(
          "category" = rep("unresolvableGenes", length(geneToCheck)),
        ...
        if (length(unique(aliases$symbol)) > 1) {
          resDF1 <- data.frame(
            "category" = rep("multiMappedGenes", length(unique(aliases$symbol))),
            "geneNames" = unique(aliases$symbol)
          )
          resDF2 <- data.frame(
            "category" = rep("resGenes", length(unique(aliases$symbol))),
            "geneNames" = unique(aliases$symbol)
          )
      ...
      resDF <- data.frame(
        "category" = rep("resGenes", length(newGene)),
        "geneNames" = newGene
      )
    }
    return(resDF)
  ...
  if (length(symbolVec) > 1) {
    ...
    resList <- list(
      "unresolvableGenes" = unresolvableGenes,
...
createGeneGuide <- function() {
  ...
  toAdd <- data.frame(
    alias_symbol = humanSymbols$symbol,
  ...
  toAdd <- data.frame(
    alias_symbol = mouseSymbols$symbol,
...
cleanInputs <- function(primaryGene = NULL,
  ...
  if (!is.null(primaryGene)) {
    ...
    if (length(res$resGenes)) {
      ...
      if (length(multiMappedGenes)) {
        ...
        showNotification(
          id = "multi-mapped-gene-warning", ui = msg, session = session,
  ...
  if (!is.null(secondaryGenes)) {
    ...
    if (length(genesetInputs) == 1) {
      ...
      res <- symbolConverter(
        symbolVec = secondaryGenes, pool = pool,
      ...
      if (length(unresolvableGenes)) {
        msg <- paste0(
          "Input list warning: '", paste0(unique(unresolvableGenes), collapse = "', '"),
          "' not found. Skipping..."
        )
        showNotification(
          id = "unresolvable-gene-warning", ui = msg, session = session,
      ...
      if (length(res$resGenes)) {
        ...
        if (length(multiMappedGenes)) {
          msg <- paste0(
            "Input '", paste0(multiMappedGenes, collapse = ", "),
            "' returned multiple official gene symbols."
          )
          showNotification(
            id = "multi-mapped-gene-warning", ui = msg, session = session,
...
createGlobalData <- function() {
  ...
  GlobalData <- list(
    "HS_basicGeneInfo" = HS_basicGeneInfo,
